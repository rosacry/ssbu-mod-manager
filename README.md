...
- **Fix Locale MSBT Files** — Detects locale-specific MSBT files (e.g. `msg_bgm+us_en.msbt`) that only work for one language and lists them on the Conflicts page. Click "Fix Locale MSBT Files" to rename them to locale-independent names (e.g. `msg_bgm.msbt`) so ARCropolis uses them as universal fallbacks
